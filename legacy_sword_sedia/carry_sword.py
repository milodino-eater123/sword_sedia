...
def senangDiriCheck(landmarks):
    left_heel = [landmarks[30].x*aspect_ratio, landmarks[30].y,landmarks[30].visibility]
    right_heel = [landmarks[29].x*aspect_ratio, landmarks[29].y,landmarks[29].visibility]
    
    if left_heel[2] < 0.5 or right_heel[2] < 0.5:
        return False, ["Please make sure both your feet are visible for senang diri"]
    
    if abs(right_heel[0] - left_heel[0]) > 0.04:
        return True,[]
    else:
...
def legRaisedAttempt(landmarks):
    left_ankle = [landmarks[28].x*aspect_ratio, landmarks[28].y,landmarks[28].visibility]
    right_ankle = [landmarks[27].x*aspect_ratio, landmarks[27].y,landmarks[27].visibility]
...
def sediaCheck(landmarks):
    left_heel = [landmarks[30].x*aspect_ratio, landmarks[30].y]
    right_heel = [landmarks[29].x*aspect_ratio, landmarks[29].y]
    if abs(right_heel[0] - left_heel[0]) < 0.05:
        return True,[]
    else:
